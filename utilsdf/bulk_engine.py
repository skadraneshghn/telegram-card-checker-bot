...
from utilsdf.response_classifier import classify_response
...
<b>⏳ Remaining in Queue -»</b> <code>{task.total}</code>"""
    ...
    while not task.queue.empty():
        ...
        try:
            ...
            traceback.print_exc()
            status_raw = "Gateway Error"
            result_raw = f"{type(e).__name__}: {str(e)[:80]}"

        # Classify the outcome using smart response classifier
        status_str, result_str, is_live, is_gw_err = classify_response(status_raw, result_raw)
